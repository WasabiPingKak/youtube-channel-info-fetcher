...
class TestBatchFetchVideoDetails:
    ...
    @patch("services.live_redirect.youtube_api.YOUTUBE_API_KEY", "test-key")
    ...
    @patch("services.live_redirect.youtube_api.YOUTUBE_API_KEY", None)
    ...
    @patch("services.live_redirect.youtube_api.YOUTUBE_API_KEY", "test-key")
    @patch("services.live_redirect.youtube_api.requests.get")
    def test_request_error_returns_partial(self, mock_get):
        """請求失敗時不中斷，回傳已取得的結果"""
    ...
    @patch("services.live_redirect.youtube_api.YOUTUBE_API_KEY", "test-key")
    @patch("services.live_redirect.youtube_api.requests.get")
    def test_batches_over_50(self, mock_get):
        """超過 50 筆時應分批查詢"""
        mock_resp = MagicMock()
