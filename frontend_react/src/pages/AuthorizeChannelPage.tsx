// src/pages/AuthorizeChannelPage.tsx
...
const AuthorizeChannelPage = () => {
    ...
    const handleAuthorize = async () => {
        setLoading(true);
        try {
            // 向後端申請一次性 OAuth state，存在 Firestore 做 server-side 驗證
            const res = await fetch(`${API_BASE}/api/oauth/state`, { method: "POST" });
            if (!res.ok) throw new Error("無法產生 OAuth state");
            ...
            console.error("OAuth state 產生失敗", err);
            setLoading(false);
        }
    };
            ...
            <div className="max-w-2xl mx-auto mt-10 px-4">
                ...
                <div className="mt-6">
                    <button
                        ...
                        className={`font-semibold py-2 px-4 rounded shadow transition ${confirmed && !loading
                            ? "bg-blue-600 hover:bg-blue-700 text-white"
                            : "bg-gray-300 dark:bg-gray-700 text-gray-500 dark:text-gray-400 cursor-not-allowed"
                            }`}
                    >
                        {loading ? "準備中..." : "使用 Google 帳號登入並連結我的 Youtube 頻道"}
                    </button>
                </div>
                {/* <div className="mt-6">
          <button
