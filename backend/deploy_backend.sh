...
# ✅ Cloud Tasks：取得 Cloud Run 服務的 URL（供 task dispatch 使用）
# 先嘗試查詢已存在的服務 URL；首次部署時會為空，部署後會自動補上
CLOUD_RUN_SERVICE_URL=$(gcloud run services describe "$SERVICE_NAME" \
  --region="$REGION" \
  --format="value(status.url)" 2>/dev/null || echo "")
if [ -z "$CLOUD_RUN_SERVICE_URL" ]; then
  echo "⚠️ 無法取得服務 URL（可能是首次部署），部署後請手動設定 CLOUD_RUN_SERVICE_URL"
fi
...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --set-env-vars "^@@^INPUT_CHANNEL=${INPUT_CHANNEL}@@GOOGLE_CLOUD_PROJECT=${PROJECT_ID}@@GOOGLE_CLIENT_ID=${GOOGLE_CLIENT_ID}@@GOOGLE_REDIRECT_URI=${GOOGLE_REDIRECT_URI}@@FRONTEND_BASE_URL=${FRONTEND_BASE_URL}@@ALLOWED_ORIGINS=${ALLOWED_ORIGINS}@@WEBSUB_CALLBACK_URL=${WEBSUB_CALLBACK_URL}@@FIRESTORE_DATABASE=${FIRESTORE_DATABASE}@@GAME_ALIAS_ENDPOINT=${GAME_ALIAS_ENDPOINT}@@CLOUD_TASKS_LOCATION=${REGION}@@CLOUD_TASKS_QUEUE=websub-subscribe@@CLOUD_RUN_SERVICE_URL=${CLOUD_RUN_SERVICE_URL}"
