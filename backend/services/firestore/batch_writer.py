...
def get_meta_doc_ref(db: Client, channel_id: str):
    return db.collection("channel_data").document(channel_id).collection("channel_info").document("cache_meta")

def get_last_video_sync_time(db: Client, channel_id: str) -> datetime:
    try:
        meta_ref = get_meta_doc_ref(db, channel_id)
        doc = meta_ref.get()
        if doc.exists:
            raw_sync = doc.to_dict().get("lastVideoSyncAt")
        ...
        logger.error("🔥 無法讀取 lastVideoSyncAt: %s", e, exc_info=True)
        return None
...
def update_last_sync_time(db: Client, channel_id: str, new_videos: List[Dict]) -> str:
    ...
    try:
        ...
        get_meta_doc_ref(db, channel_id).set({"lastVideoSyncAt": latest}, merge=True)
